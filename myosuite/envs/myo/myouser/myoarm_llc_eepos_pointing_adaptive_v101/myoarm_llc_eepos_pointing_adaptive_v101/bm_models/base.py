...
class BaseBMModel(ABC):
  ...
  def get_assets_dir(cls):
    """ Overwrite this method if the assets are stored somewhere different than in a subdirectory called 'assets'. """
    return os.path.join(parent_path(inspect.getfile(cls)), "assets")
  ...
  def clone(cls, simulator_folder, package_name):
    ...
    # Copy assets
    shutil.copytree(cls.get_assets_dir(), os.path.join(simulator_folder, package_name, "assets"),
  ...
  def insert(cls, simulator_tree):
    ...
    # Add defaults
    ETutils.merge_children("default", bm_root, simulator_root)
