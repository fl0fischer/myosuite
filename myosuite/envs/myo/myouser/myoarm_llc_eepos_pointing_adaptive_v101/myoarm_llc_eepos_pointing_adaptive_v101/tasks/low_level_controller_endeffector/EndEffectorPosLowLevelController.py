...
class EndEffectorPosLowLevelController(BaseTask):
  ...
  def __init__(self, model, data, end_effector, shoulder, **kwargs):
    ...
    self._target_origin = getattr(data, self._shoulder[0])(self._shoulder[1]).xpos + np.array([0.225, -0.1, 0.05]) 
    self._target_position = self._target_origin.copy()
    self._target_limits_x = np.array([-0.125, 0.125])
    self._target_limits_y = np.array([-0.2, 0.2])
    self._target_limits_z = np.array([-0.35, 0.35])
  ...
  def _spawn_target(self, model, data, new_position=None, new_radius=None):
    ...
    if new_position is None:
      ...
      for _ in range(10):
        target_x = self._rng.uniform(*self._target_limits_x)
